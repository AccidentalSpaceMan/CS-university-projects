...
AST::AST(NodeType t, const std::string& s) {
    type = t;
    switch (type) {
        case category:
            tag = s;
...
AST::AST(const AST& actual) {
    ...
    for (AST *childNode : actual.child) {
        child.push_back(new AST(*childNode));
    }
}
...
void AST::swap(AST& rhs) {
    ...
    NodeType t_type = type;
    type = rhs.type;
    rhs.type = t_type;
    std::string t_tag = tag;
    tag = rhs.tag;
    rhs.tag = t_tag;
    std::string t_text = text;
    text = rhs.text;
    rhs.text = t_text;
    std::string t_closeTag = closeTag;
    closeTag = rhs.closeTag;
    rhs.closeTag = t_closeTag;
}
...
// REQUIRES: this->type == category && tagName is valid srcML tag
...
void AST::insertMainHeader(const std::vector<std::string>& profileName,
    ...
    while (obj != child.end() && (*obj)->tag != "function") {
        obj++;
    }

    std::string block = "\n#include \"profile.hpp\"\n";
    for (int i = 0; i < (int)profileName.size(); ++i) {
        block += "profile " + profileName[i] + "(\"" + fileName[i] + "\");\n";
    }
    block += "\n";
    child.insert(obj, new AST(token, block));
...
void AST::insertFileHeader(const std::string& profileName) {
    ...
    while (((*obj)->tag != "function") && obj != child.end() ) {
    	obj++;
    }
...
void AST::insertMainReport(const std::vector<std::string>& profileName) {
    ...
    for (AST* node : child) {
        if (node->tag == "function") {
            ...
            if (nameNode->getName() == "main") {
                AST* block        = node->getChild("block");
                AST* blockContent = block->getChild("block_content");

                // Search backwards from end for the return statement
                std::list<AST*>::iterator it = blockContent->child.end();
                --it;
                while (it != blockContent->child.begin() && (*it)->tag != "return") {
                    --it;
                }

                // Insert a cout for each profile object before the return
                for (const std::string& p : profileName) {
                    std::string report = "\n    std::cout << " + p + " << std::endl;\n";
                    blockContent->child.insert(it, new AST(token, report));
                }
                return;
            }
        }
...
void AST::insertFunctionCount(const std::string& profileName) {
    ...
    for (AST* node : child) {
        if (node->tag == "function"    ||
            ...
            std::string count = " " + profileName + ".count(__LINE__, \"" + funcName + "\");";
            blockContent->child.push_front(new AST(token, count));
        }
    }
}
    ...
    while (it != child.end()) {
        if ((*it)->type == category) {
            if ((*it)->tag == "expr_stmt") {
                ...
                child.insert(it, new AST(token, count));
            } else if (!isStopTag((*it)->tag)) {
                (*it)->insertLineCount(profileName);
                ++it;
            } else {
                ++it;
            }
...
std::ostream& AST::print(std::ostream& out) const {
    for (AST* childNode : child) {       //For each AST* in child
        if (childNode->type != category)
            out << childNode->text;      //token || whitespace node, print it
        else
            childNode->print(out);       //category node, recursive call
    }
    return out;
}
    ...
    while (!in.eof()) {
        if (ch == '<') {                      //Found a tag
            ...
            for (std::string t : tokenList) {
                if (isspace(t[0])) {
                    subtree = new AST(whitespace, t);
                } else {
                    subtree = new AST(token, t);
                }
